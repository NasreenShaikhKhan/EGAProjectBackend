...
public class AuthenticationController {
    ...
    public ResponseEntity<?> createAuthenticationToken(@RequestBody JwtRequest authenticationRequest) throws Exception {
        authenticate(authenticationRequest.getAccountNo(), authenticationRequest.getPassword());
        System.out.println("Entered");
        final UserDetails userDetails = userDetailsService.loadUserByUsername(authenticationRequest.getAccountNo());
        final String token = jwtTokenUtil.generateTokenForLogin(userDetails);
        User user = userService.getUserFromToken(token);
        UserDto userDto=UserDto.builder().name(user.getName()).email(user.getEmail()).accountNo(user.getAccount().getAccountNo()).build();
